...
TEST(VectorVectorVectorFloat, ThreeDimensionalVector) {
    ...
    std::vector<std::vector<std::vector<float>>> vector{{{1, 2}, {3, 4}},
                                                        {{5, 6}, {7, 8}}};
...
TEST(ForwardListFloat, InsertElementAfterIndex) {
    ...
    auto pos = forwardList.begin();
    std::advance(pos, 1);
    forwardList.insert_after(pos, 10);
